...
SCOPE, stated up front: this selector covers DIAGONALLY-INVARIANT predicates
(live(q,kv) <=> q-kv in D). The shared test set also lists sinks, docpack and
bidoc, which are not diagonally invariant; those are NOT covered and are reported
as out-of-scope rather than silently skipped.
"""
import random, time
from blocks import Iv, Ap
from spec import DiagSpec
from xform import CANDIDATES, cost_of, select
from oracle import oracle_select, oracle_cost, dense, apply_xform, tiles_cost
...
def rand_D(N, kind):
    if kind == "runs":
        ...
        return pieces
    if kind == "scatter":                      # irregular D: 300 random offsets
        offs = sorted(rng.sample(range(N), min(300, N)))
        return [Iv(o, o + 1) for o in offs]
...
print(f"{'N':>6}{'cells':>7}{'agree':>8}{'meanReg':>9}{'maxReg':>8}"
      f"{'declined-but-costable':>24}")
allmax, worst_case = 1.0, None
for N in (1024, 1536, 2048):
    cells = agree = 0; regs = []; dec = 0
    for name, pieces in build_suite(N):
        ...
        for BQ in TILES:
            for A in TILES:
                if N % BQ or N % A:
                    continue
                for c in CANDIDATES:            # coverage: do we decline something real?
                    if cost_of(c, D, N, BQ, A) is None and \
                       oracle_cost(pieces, N, BQ, A, c) is not None:
                        dec += 1
                pick, pc = select(D, N, BQ, A)
                opick, oc = oracle_select(pieces, N, BQ, A, CANDIDATES)
                if pick is None or oc is None:
                    continue
                cells += 1
                agree += (pick == opick)
                true_pick = oracle_cost(pieces, N, BQ, A, pick)
                r = true_pick / oc if oc else 1.0
                regs.append(r)
                if r > allmax:
                    allmax, worst_case = r, (name, N, BQ, A, pick, opick, r)
    print(f"{N:>6}{cells:>7}{agree/cells*100:>7.1f}%{sum(regs)/len(regs):>9.4f}"
          f"{max(regs):>8.4f}{dec:>24}")
if worst_case:
    n, N, BQ, A, p, o, r = worst_case
    print(f"  worst regret {r:.4f}: {n} N={N} {BQ}x{A} picked {p}, oracle {o}")
else:
    print("  worst regret 1.0000 -- selector never picked a suboptimal transform")
